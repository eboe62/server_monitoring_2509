...
if [ -z "${NVIDIA_NIM_API_KEY:-}" ]; then
    ...
    echo "👉 Ejecuta primero: export NVIDIA_NIM_API_KEY=\"tu_clave_nvapi_aquí\""
    echo "💡 Puedes conseguir una clave gratuita registrándote en https://build.nvidia.com"
    exit 1
fi
...
# 3. Lanzamiento automatizado con Hardening Estricto y restricción Loopback al puerto 8082 y Enrutamiento hacia NVIDIA NIM
# Modificación en proxy-start-gemini.sh

# 1. Asegúrate de que la clave de Gemini esté disponible
if [ -z "${GEMINI_API_KEY:-}" ]; then
    echo "❌ Error: La variable GEMINI_API_KEY no está exportadas en el host."
    exit 1
fi

# 2. Recrear el contenedor apuntando a Gemini Pro
docker run -d \
  --name ia_opencode_proxy \
  --network ia-net \
